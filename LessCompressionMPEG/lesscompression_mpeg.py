...
def main(anchorFrame, targetFrame, saveOutput=False, blockSize=16):
    bitsAnchor = []
    bitsDiff = []
    bitsPredicted = []
    ...
    if not isdir:
        os.mkdir('Results')
    if saveOutput:
        cv2.imwrite(f"{'Results'}/First frame.png", anchorFrame)
        cv2.imwrite(f"{'Results'}/Second frame.png", targetFrame)
        cv2.imwrite(f"{'Results'}/Difference between frame.png", diffFrameRGB)
        cv2.imwrite(f"{'Results'}/Prediction frame.png", predictedFrameRGB)
        cv2.imwrite(f"{'Results'}/Residual frame.png", residualFrameRGB)
        cv2.imwrite(f"{'Results'}/Restore frame.png", restoreFrameRGB)
    barWidth = 0.25
    ...
    plt.legend()
    plt.savefig(f'{"Results"}/Гістограма кількості біт на піксель для різних варіантів кодування.png', dpi=600)
    return None


if __name__ == "__main__":
    fr = random.randint(0, 3000)
    frame1, frame2 = getFrames('C:/Image/sample4.avi', fr, fr + 1)
